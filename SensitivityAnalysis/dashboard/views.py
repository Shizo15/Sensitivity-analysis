        ...
        if not video_id:
            ...
            if not is_allowed:
                ...
                if model_name not in MODEL_CATALOG and model_name != 'roberta':
                    messages.error(request, f"The selected model ({model_name}) is unavailable.")
                    return redirect('sentiment_dashboard')

                # RESET PROGRESS BAR STATUS BEFORE STARTING
                request.session['analysis_status'] = {'progress': 0, 'step': 'Initializing...'}
